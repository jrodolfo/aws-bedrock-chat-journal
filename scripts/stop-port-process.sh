...
What it does:
  Stops the process listening on the selected TCP port.
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
    echo "On Windows, use ./scripts/stop-port-process.ps1 instead of ./scripts/stop-port-process.sh." >&2
    exit 1
    ;;
esac
...
kill "${pid}"
echo "Stopped process ${pid} listening on port ${PORT}."
